defmodule XSD.Facet do
  ...
  defmacro __using__(opts) do
    ...
    quote bind_quoted: [], unquote: true do
      ...
      @doc """
      Returns the value of this `XSD.Facet` on specific `XSD.Datatype`.
      """
      @callback unquote(name)() :: unquote(type_ast) | nil

      @doc """
      Validates if a `value` and `lexical` conforms with a concrete `facet_constaint_value` for this `XSD.Facet`.

      This function must be implemented on a `XSD.Datatype` using this `XSD.Facet`.
      """
      ...
      @doc """
      Checks if a `value` and `lexical` conforms with the `c:#{unquote(conform_fun_name(name))}/3` implementation on the `datatype` `XSD.Datatype`.
      """
      @spec conform?(XSD.Datatype.t(), any, XSD.Datatype.uncanonical_lexical()) :: boolean
  ...
  @doc """
  Macro for the definition of concrete constraining `value` for a `XSD.Facet` on a `XSD.Datatype`.
  """
